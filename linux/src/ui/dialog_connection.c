...
static void on_connection_button_press(GtkWidget *button, gpointer data) {
  ...
  if (ctx->db_type == POSTGRESQL) {
    ...
    const char *db = gtk_editable_get_text(GTK_EDITABLE(form->db));

    if (strlen(host) == 0 || strlen(port) == 0 || strlen(username) == 0)
      return;

    info = connection_info_new(conn_name, DB_TYPE_POSTGRESQL, host, port,
                               username, password, db);
  } else if (ctx->db_type == DB_MYSQL) {
    const char *host = gtk_editable_get_text(GTK_EDITABLE(form->host));
    const char *port = gtk_editable_get_text(GTK_EDITABLE(form->port));
    const char *username = gtk_editable_get_text(GTK_EDITABLE(form->username));
    const char *password = gtk_editable_get_text(GTK_EDITABLE(form->password));
    ...
    if (strlen(host) == 0 || strlen(username) == 0)
      return;

    info = connection_info_new(conn_name, DB_TYPE_MYSQL, host, port, username,
                               password, db);
  } else if (ctx->db_type == SQLITE) {
    const char *db = gtk_editable_get_text(GTK_EDITABLE(form->db));

    if (strlen(db) == 0)
      return;

    info = connection_info_new(conn_name, DB_TYPE_SQLITE, "", "", "", "", db);
  }

  if (info && ctx->sidebar->conn_store) {
    connection_store_add(ctx->sidebar->conn_store, info);
    connection_store_save(ctx->sidebar->conn_store);
    sidebar_add_connection(ctx->sidebar, info);
  }

  GtkWidget *parent = GTK_WIDGET(gtk_widget_get_root(button));
...
static GtkWidget *window_form_pg(SqlForm *form, GtkWidget **btn_connect_out) {
  ...
  gtk_window_set_transient_for(GTK_WINDOW(dialog),
                               GTK_WINDOW(gtk_widget_get_root(dialog)));
  ...
  GtkWidget *widgets[] = {label,    conn_name, host, port,
                          username, password,  db,   btn_connect};
...
static GtkWidget *window_form_mysql(SqlForm *form,
                                    GtkWidget **btn_connect_out) {
  GtkWidget *dialog = gtk_dialog_new();
...
static GtkWidget *window_form_sqlite(SqlForm *form,
                                     GtkWidget **btn_connect_out) {
  GtkWidget *dialog = gtk_dialog_new();
  gtk_window_set_title(GTK_WINDOW(dialog), "SQLite Connection");
  gtk_window_set_modal(GTK_WINDOW(dialog), TRUE);
  gtk_window_set_transient_for(GTK_WINDOW(dialog),
                               GTK_WINDOW(gtk_widget_get_root(dialog)));
  ...
  GtkWidget *label = gtk_label_new("SQLite Connection Details");
  GtkWidget *conn_name = gtk_entry_new();
  gtk_entry_set_placeholder_text(GTK_ENTRY(conn_name), "Connection Name");
  GtkWidget *db_path = gtk_entry_new();
  gtk_entry_set_placeholder_text(GTK_ENTRY(db_path), "Database File Path");
  ...
  GtkWidget *widgets[] = {label, conn_name, db_path, btn_connect};
  for (int i = 0; i < G_N_ELEMENTS(widgets); i++) {
    gtk_box_append(GTK_BOX(content), widgets[i]);
  };
...
static void on_db_selected(GtkWidget *button, gpointer data) {
  ...
  case POSTGRESQL:
    ...
    g_signal_connect(btn_connect, CLICKED,
                     G_CALLBACK(on_connection_button_press), ctx);
  ...
  case SQLITE:
    dialog = window_form_sqlite(form, &btn_connect);
    ctx = g_malloc(sizeof(SqlConnectContext));
    ctx->form = form;
    ctx->sidebar = sidebar;
    ctx->conn_store = sidebar->conn_store;
...
void show_create_connection_dialog(GtkWidget *widget, gpointer data) {
  ...
  GtkWidget *buttons[] = {create_menu_button(DB_TYPE_NAMES[POSTGRESQL], NULL),
  ...
  g_signal_connect_swapped(buttons[3], CLICKED, G_CALLBACK(gtk_window_destroy),
                           dialog);
